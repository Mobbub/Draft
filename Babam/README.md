...
Далее мы должны на своём локальном хосте залить бд из db_expo_part и настроить файл config.py (под себя, под свой хост). И также необходитмо создать пустую таблицу, где будут распологаться бд с клиентами. Можно использовать MySQL Workbench
