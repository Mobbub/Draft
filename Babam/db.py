...
class ClientsDatabase(MySQLConnection):
    ...
    def fetch_data_from_file(self, db_file_path: str) -> dict:
        """Определить тип файла и извлечь данные, вернув их в формате JSON."""
        ...
        if db_file_path.endswith('.db'):
            loaded_data = self._fetch_data_from_sqlite(db_file_path).copy()
        elif db_file_path.endswith('.sql'):
            loaded_data = self._process_sql_dump(db_file_path).copy()
        else:
            raise ValueError("Неподдерживаемый формат файла")

        formatted_data = self._process_data(loaded_data)

        # Запуск отдельного потока, чтобы не ждать, пока данные запишутся в таблицу
        threading.Thread(target=self._insert_data_in_mysql).start()
    ...
    def _insert_data_in_mysql(self) -> None:
