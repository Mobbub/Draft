...
class WorkDB: ###
    ...
    def add_zap(self, add_zap): ###
        
        with open(self.file_path, 'a', encoding='utf-8') as file:
            file.write(str(add_zap) + '\n')
        print('Вы успешно добавили запись в файл')
...
class Main:
    def __init__(self):
        self.actions = ['1', '2', '3', '4', '1.', '2.', '3.', '4.', 'вывод баланса', 'добавить записи', 'редактирование записи', 'поиск по записям']
        self.add_zap = {
                'Номер записи': {
                    'Дата': '',
                    'Категория': '',
    ...
    def main(self, deytv): ###
        work_db = WorkDB()
        if deytv in ['1', '1.', 'вывод баланса']:
            return f'Ваш балланс равен: {WorkDB.bal}' ###
        elif deytv in ['2', '2.', 'добавить записи']:
            user_input = input('Выберите категорию записи:\n1. Расход\n2. Доход\n> ')
            if user_input in ['1', '1.', 'расход']:
                self.add_zap['Номер записи']['Категория'] = 'Расход'
            elif user_input in ['2', '2.', 'доход']:
                self.add_zap['Номер записи']['Категория'] = 'Доход'
            self.add_zap['Номер записи']['Сумма'], self.add_zap['Номер записи']['Описание'] = input('Введите сумму:\n> '), input('Введите описание:\n> ')
            work_db.add_zap(self.add_zap) ###
        elif deytv in ['3', '3.', 'редактирование записи']:
            return "Редактирование записи"
        elif deytv in ['4', '4.', 'поиск по записям']:
